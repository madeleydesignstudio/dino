...
export const LocationTimeDisplay = ({
  ...
  return (
    <div className={`absolute inset-0 pointer-events-none ${className}`}>
      ...
      <div className="absolute bottom-0 right-4 text-xs text-gray-500 font-mono tracking-wide flex items-center">
        {location}
      </div>

      {/* Time - Bottom Right (in py-6 bottom padding zone) */}
      <div className="absolute top-0 right-4 pt-8 text-xs text-gray-500 font-mono tracking-wide flex items-center">
        {time}
      </div>
    </div>
  );
