...
export function ToolCard({ tool }: ToolCardProps) {
  ...
  if (typeof tool === "string") {
    return (
      <motion.div
        key={tool}
        className="group relative p-6 bg-gradient-to-br from-white/50 to-white/30 dark:from-stone-800/50 dark:to-stone-800/30 backdrop-blur-sm rounded-2xl border border-stone-200/50 dark:border-stone-700/50 shadow-sm hover:shadow-md transition-all duration-300"
  ...
  // Updated rendering for object type tools
  return (
    <>
      <motion.div
        key={tool.name}
      ...
      >
        <div className="flex items-center gap-4">
          {tool.logo && (
            <div className="relative flex-shrink-0 w-12 h-12 bg-white dark:bg-stone-700 rounded-xl p-2.5 shadow-sm group-hover:shadow transition-shadow duration-300">
              <img
                src={tool.logo}
                alt={`${tool.name} logo`}
        ...
        <DialogContent className="sm:max-w-xl">
          <DialogHeader className="space-y-4">
            <DialogTitle className="flex items-center gap-4">
              {tool.logo && (
                <div className="relative flex-shrink-0 w-14 h-14 bg-white dark:bg-stone-700 rounded-xl p-3 shadow-sm">
                  <img
                    src={tool.logo}
                    alt={`${tool.name} logo`}
                    className="w-full h-full object-contain"
